...
fis.config.merge({
    modules: {
        ...
        postpackager : 'modjs', //保证index.html中的require.async能运行（当然也要生成require.resourcemap）生成map.js一份资源列表和map.json内容一直，只不过加上了 require.resourcemap,
        packager: 'map',
        spriter: 'csssprites',
        optimizer : {
            js: 'uglify-js',
    ...
    settings: {
        ...
        postpackager : {
             modjs : {
                subpath : 'map/map.js' // 样式文件会 导入到 map.js中去，require.resourceMap会这样加载<script src="/index/index.css"></script>导致错误
            }
        },
        spriter: {
            csssprites: {
